# -*- coding: utf-8 -*-
"""
Created on Wed Oct  8 11:17:17 2025
...
#Create checklist for statute scraping by state.
if os.path.isfile('checklist_v1.csv'):
    checklist=pd.read_csv('checklist_v1.csv')
else:
    checklist=pd.DataFrame(states,columns=['state'])
    checklist=checklist.reindex(columns=['state','Completed','Paragraph Ready','Manual','Multi Issue','Other'])

#Load main_df and checklist.
#main_df=pd.read_csv('current_text_v1.csv')
#checklist=pd.read_csv('checklist_v1.csv',index=False)
if os.path.isfile('current_text_v1.csv'):
    main_df=pd.read_csv('current_text_v1.csv')
else:
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['No','Yes','No','No','No']
checklist.to_csv('checklist_v1.csv',index=False)
...
temp_df.loc[temp_df.temp_statute=='nan','temp_statute']=temp_df.loc[temp_df.temp_statute=='nan','backsources_2023'].str.split('AS ',expand=True)[1]
temp=temp_df['temp_statute'].str.split(stat_split[s],expand=True)
print(f"\nNumber of columns after split: {temp.shape[1]}")
temp.columns=['title','chapter','section']
temp['state']=s
temp=temp[['state','title','chapter','section']]
temp.loc[pd.isnull(temp_df.temp_statute)==True,['title','section']]=['0','0']
temp['paragraph']=temp['section'].str.extract('\((.*)',expand=True)
temp['paragraph']='('+temp['paragraph']
temp['section']=temp['section'].str.split('(',expand=True)[0]
temp['section_url']=temp_df['temp_statute'].str.split('(',expand=True)[0]
...
temp['url']=temp['url']+'/'+'section-'+temp['section_url']+'/'
temp['url']=url_base+url_state_year+temp['url']
temp.loc[pd.isnull(temp_df.temp_statute)==True,'url']='https://regulations.justia.com/states/alaska/title-9/chapter-12/section-9-aac-12-010/'
...
for i in temp.index:
    ...
    if pd.isnull(temp.loc[i,'section_url'])==False:
        ...
        temp.loc[i,'statute_mods']='Authority:AS 45.68.010 AS 45.68.020 AS 45.68.130'
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['No','No','No','Yes','One obs. with only URL, no statute.']
checklist.to_csv('checklist_v1.csv',index=False)
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['No','Yes','No','No','Paragraph letters are capitalized and followed by a period.']
checklist.to_csv('checklist_v1.csv',index=False)

# %%California.  Partially on findlaw.
s='california'
stat_split[s]=' '
url_state_year=s+'/2023/'
...
temp['state']=s
temp[['cal_code','title','division','part','chapter','article']]=np.nan

temp.loc[((temp_df.backsources_2023.str.contains('Ann.Cal.Gov.Code'))|(temp_df.backsources_2023.str.contains('Cal. Gov. Code'))|(temp_df.backsources_2023.str.contains('CA GOVT'))) ,'cal_code']='gov'
temp.loc[((temp_df.backsources_2023.str.contains('Ann.Cal.Bus.Code')) & (temp.section=='17510.3')),'cal_code']='bpc'
temp.loc[pd.isnull(temp.cal_code)==True,'cal_code']='corp'

temp.loc[temp.cal_code=='gov',['title','division','part','chapter','article']]=['2','3','2','6','7']
temp.loc[temp.cal_code=='bpc',['title','division','part','chapter','article']]=['','7','3','1','1-3']
temp=temp[['state','cal_code','title','division','part','chapter','article','section']]
...
#Create url for each statute.
temp.loc[temp.cal_code=='gov','url']='code-'+temp['cal_code']+'/title-'+temp['title']+'/division-'+temp['division']+'/part-'+temp['part']+'/chapter-'+temp['chapter']+'/article-'+temp['article']+'/section-'+temp['section_url']+'/'
temp.loc[temp.cal_code=='bpc','url']='code-'+temp['cal_code']+'/division-'+temp['division']+'/part-'+temp['part']+'/chapter-'+temp['chapter']+'/article-'+temp['article']+'/section-'+temp['section_url']+'/'
temp.loc[temp.cal_code!='corp','url']=url_base+url_state_year+temp['url']
temp.loc[temp.cal_code=='corp','url']='https://codes.findlaw.com/ca/corporations-code/corp-sect-'+temp['section']+'/'
    ...
    if temp.loc[i,'section_url']!='nan':
        ...
        if temp.loc[i,'cal_code']!='corp':
            ...
            if mod_text is None:
                ...
                mod_text=mod_text.get_text()
            text=text.get_text(separator='<em>', strip=False).split('<em>')
            temp.loc[i,'statute_text']=''.join(text[:-1])
            temp.loc[i,'statute_text']=temp.loc[i,'statute_text'].replace('\xa0',' ')
            temp.loc[i,'statute_mods']=''.join(mod_text)
            
            #This gets the universal citation.
            temp.loc[i,'universal_citation']=soup.find('div',class_='citation').find('span').get_text()
        else:
            text=soup.find('div', class_='codes-content__text codes-content__text--min-height').get_text()
            temp.loc[i,'statute_text']=text.replace('\t','').replace('\n','').replace('   ',' ')
            temp.loc[i,'statute_mods']=''
            
            #This gets the universal citation.
            if soup.find('p',class_='fl-block-paragraph').get_text().find('January 01, 2024')!=-1:
                temp.loc[i,'universal_citation']='CA Corp Code § '+temp.loc[i,'section']+' (2023)'
            else:
                temp.loc[i,'universal_citation']='CA Corp Code § '+temp.loc[i,'section']
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['No','Yes','No','No','One obs. is the audit threshold.']
checklist.to_csv('checklist_v1.csv',index=False)
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['No','Yes','No','No','One obs. with only URL, no statute.']
checklist.to_csv('checklist_v1.csv',index=False)
...
for i in temp.index:
    if temp.loc[i,'section_url']!='nan':
        ...
        #This separates text into statute and previous amendments to statute.
        text=soup.find('div',id='codes-content')
        text=text.get_text(strip=True).split('History:')
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['No','Yes','No','No','One obs. with only URL, no statute. One obs. refers to Solicitation of Charitable Funds Act, C.G.S. chapter 419d']
checklist.to_csv('checklist_v1.csv',index=False)
...
temp=pd.DataFrame(columns=['Title','Chapter','Subchapter','Section'])



checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['No','No','Yes','Yes','No']
checklist.to_csv('checklist_v1.csv')

# %%DC.
stat_split['washington, d.c.']='-'
s='washington, d.c.'
url_state_year='district-of-columbia'+'/2023/'
temp_df=current_df.loc[current_df.state==s]
print(np.sum(temp_df['MULTI']=='CHECK §§'))
print(temp_df.loc[temp_df.temp_statute=='nan','backsources_2023'])
...
temp.columns=['title','section']
temp['state']=s
temp[['chapter','subchapter','part']]=np.nan
temp['DCMR']='no'
temp=temp[['state','title','chapter','subchapter','part','DCMR','section']]
temp.loc[pd.isnull(temp.section),'DCMR']='yes'
temp.loc[temp.DCMR=='yes','section']=temp['title']
temp.loc[temp.DCMR=='yes','title']='16'

temp.loc[temp.title=='29','chapter']='4'
temp.loc[temp.title=='44','chapter']=temp.loc[temp.title=='44','section'].str.extract('(\d+)')[0]
temp.loc[((temp.title=='44') & (temp.chapter.str.len()==3)),'chapter']=temp.chapter.str[:1]
temp.loc[((temp.title=='44') & (temp.chapter.str.len()==4)),'chapter']=temp.chapter.str[:2]

subchapter_list=['i','ii','iii','iv','v','vi','vii','viii','ix','x','xi','xii','xiii','xiv']
temp.loc[((temp.title=='29') & (temp.section.str[:3].astype('int')>409)),'subchapter']=temp.section.str[1:3]
temp.loc[((temp.title=='29') & (temp.section.str[:3].astype('int')<=409)),'subchapter']=temp.section.str[2]
for i in temp.loc[temp.title=='29'].index:
    temp.loc[i,'subchapter']=subchapter_list[int(temp.loc[i,'subchapter'])-1]

temp.loc[((temp.subchapter=='ix')|(temp.subchapter=='x')|(temp.subchapter=='xi')),'part']=''
temp.loc[((temp.title=='29') & (temp.section=='401.01')),'part']='a'
temp.loc[((temp.title=='29') & (temp.section=='406.09')),'part']='a'
temp.loc[((temp.title=='29') & (temp.section=='408.06')),'part']='a'
temp.loc[((temp.title=='29') & (temp.section.str.contains('412.0'))),'part']='a'
temp.loc[((temp.title=='29') & (temp.section.str.contains('412.2'))),'part']='b'
...
temp.loc[temp.title=='44','url']='title-'+temp['title']+'/chapter-'+temp['chapter']+'/section-'+temp['section_url']+'/'
temp.loc[((temp.title=='29') & (temp.part=='')),'url']='title-'+temp['title']+'/chapter-'+temp['chapter']+'/subchapter-'+temp['subchapter']+'/section-'+temp['section_url']+'/'
temp.loc[((temp.title=='29') & (temp.part!='')),'url']='title-'+temp['title']+'/chapter-'+temp['chapter']+'/subchapter-'+temp['subchapter']+'/part-'+temp['part']+'/section-'+temp['section_url']+'/'
temp['url']=url_base+url_state_year+temp['url']
...
for i in temp.index:
    if temp.loc[i,'DCMR']=='no':
        ...
        for j in range(len(text)):
            if ((text[j].startswith('(')==True) & (text[j].endswith(')')==True)):
                temp.loc[i,'statute_text']=''.join(text[:j])
                temp.loc[i,'statute_mods']=text[j]

        #This gets the universal citation.
        temp.loc[i,'universal_citation']=soup.find('div',class_='citation').find('span').get_text()

#Manual DCMR text.
temp.loc[((temp.title=='16') & (temp.section=='1304')),['url','statute_text','statute_mods','universal_citation']]=['https://www.dcregs.dc.gov/Common/DCMR/SectionList.aspx?SectionNumber=16-1304',"1304.1	Application for a certificate of registrations, under the provisions of § 4 of the Act [D.C. Official Code § 44-1703(a) (2001 Ed.)] shall be made in accordance with the provisions of this section. 1304.2	Each application for a certificate of registration shall contain the following information: (a)	The name, address, and headquarters of the applicant;(b)	If incorporated, the date and place of incorporation;(c)	If the applicant is not an individual, the names and headquarters of the applicant's principal officers and managers and a copy of the resolution authorizing the person or persons making application for the certificate of registration on behalf of the applicant to take that action, certified to as a true and correct copy by the person having charge of the applicant's records;(d)	A copy of the charter and by-laws of the applicant;(e)	The purposes of the solicitation to be made under the authority of the certificate of registration applied for, the estimated total of funds proposed to be raised by the solicitation, and all the uses or dispositions to be made of the funds;(f)	The names and addresses of all persons who will have responsibility for managing the solicitations, including managerial personnel both within and outside the District;(g)	The names and addresses of all persons by whom the receipts or proceeds of the solicitation may be disbursed;(h)	An outline of the method or methods to be used in conducting the solicitation;(i)	The time when solicitations are to be made, giving the dates on which solicitations are to begin and end;(j)	The total estimated cost of the solicitation, including all costs of administration, promotion, solicitations, collection, and disbursement; and(k)	A statement of the estimated cost of the solicitation, expressed as a percentage of the estimated total amount expected to be collected. 1304.3	If professional fundraisers or solicitors will be connected with the solicitation to any extent, the following information shall be provided with the application: (a)	The names and addresses of all professional fund raisers and professional solicitors; (b)	The amount of wages, fees, commissions, expenses, or emoluments to be paid to each professional fund raiser and professional solicitor; and (c)	A copy of the contract, if any, made by or on behalf of the applicant with each professional fund raiser and professional solicitor. 1304.4	Each application shall be accompanied by an itemized financial statement for the last preceding calendar year or fiscal year, as the records of applicant may be kept, of any funds collected in the District of Columbia through a charitable solicitation by or on behalf of the applicant. This statement shall disclose the amount of money so collected, together with the costs of collecting, and the final distribution of the funds collected. 1304.5	If the applicant completes an application for renewal of registration prior to the expiration of a registration period the applicant need not file an itemized financial statement under § 1304.4, except upon demand of the Director. Nothing in this section shall absolve the applicant from compliance with the reporting requirements of § 7 of the Act (D.C. Official Code § 44-1706 (2001 Ed.)). 1304.6	Each application shall be accompanied by a full statement of the character and extent of the charitable work being done by the applicant in the District. 1304.7	Each application shall be accompanied by a statement to the effect that if the certificate of registration is granted, the certificate will not be used or represented in any way as an endorsement by the District or by the Department of the solicitation to be conducted under the authority of the certificate. 1304.8	If the applicant is a non-resident, the applicant shall appoint and maintain an attorney-in-fact to act as the applicant's agent in the District for the service of any notice or process authorized by the Act or this chapter. 1304.9	If the applicant is unable to furnish any of the information required by this section, a detailed statement shall be given the Director of the reason or reasons why the information cannot be furnished. 1304.10	The fee for a certificate of registration shall be eighty dollars ($ 80) which shall be paid at the time application is made.",'1/1/1900','16 DCMR § 1304']
temp.loc[((temp.title=='16') & (temp.section=='1304.3')),['url','statute_text','statute_mods','universal_citation']]=['https://www.dcregs.dc.gov/Common/DCMR/SectionList.aspx?SectionNumber=16-1304','1304.3	If professional fundraisers or solicitors will be connected with the solicitation to any extent, the following information shall be provided with the application: (a)	The names and addresses of all professional fund raisers and professional solicitors; (b)	The amount of wages, fees, commissions, expenses, or emoluments to be paid to each professional fund raiser and professional solicitor; and (c)	A copy of the contract, if any, made by or on behalf of the applicant with each professional fund raiser and professional solicitor.','1/1/1900','16 DCMR § 1304.3']
temp.loc[((temp.title=='16') & (temp.section=='1306')),['url','statute_text','statute_mods','universal_citation']]=['https://www.dcregs.dc.gov/Common/DCMR/SectionList.aspx?SectionNumber=16-1306',"1306.1	Each registrant and each applicant for a certificate of registration shall make application to the Director for the number of solicitor information cards that will be required to conduct the solicitation. 1306.2	At the request of persons exempt under §§ 4(b), (c), and (d) of the Act, the Director shall issue to them identification cards to be distributed to each solicitor employed by that exempt person. 1306.3	Solicitor information cards shall be supplied at cost. 1306.4	Each solicitor information card shall be serially numbered and shall bear following information:(a)	The name and address of the solicitor;(b)	The number of certificate of registration under the authority of which the solicitor solicits contributions;(c)	The name of the registrant;(d)	The dates within which solicitations may be made;(e)	A statement that the card does not constitute an endorsement of the solicitation by the District of Columbia, or by any officer or employee of the District; and(f)	Other information or conditions that the Director may require. 1306.5	The Director may, in his or her discretion, issue solicitor information cards with the names of the solicitors omitted. If the cards are issued without names, the names of the solicitors shall subsequently be furnished to the Director at the Director's request. 1306.6	The Director may, in connection with any request for solicitor information cards, require the registrant to submit a list in triplicate of the names and addresses of the persons for whom the cards are being requested, and the Director may either prepare or cause to be prepared a solicitor information card for each name appearing on the list, or the Director may serially number the names of the list and issue like-numbered cards to the registrant, for completion in accordance with the requirements of this section. 1306.7	No registrant to whom the Director issues serially numbered cards against a serially numbered list of names shall issue any card to any person other than the person indicated on the list of names by the same number as that appearing on a card. 1306.8	No registrant issued a completely filled-out solicitor information card shall distribute the card to any person other than the person named on the card. 1306.9	No person other than the person whose name appears on the solicitor information card shall be issued that card or be authorized to use the card by a person whose name does not appear on the card, even if the person is employed by a registrant to make solicitations, shall be a violation of this chapter. 1306.10	Each registrant shall be responsible for carrying out and securing compliance with the provisions of this section and § 1307. Failure or refusal of any registrant or solicitor to comply with the provisions of these sections shall be grounds for the suspension or revocation of the certificate of registration issued to the registrant.",'1/1/1900','16 DCMR § 1306']
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['No','No','Yes','No','DC ST statutes are on Justia, and DCMR statutes are done manually. For DC ST § 29-401.01 et seq mean all of 401 or something else?']
checklist.to_csv('checklist_v1.csv',index=False)
...
temp_df.loc[temp_df.temp_statute=='nan','temp_statute']=temp_df.loc[temp_df.temp_statute=='nan','backsources_2023'].str.split('Fla. Stat. Ann. ',expand=True)[1]
temp_df.loc[temp_df.temp_statute.str.contains('986'),'temp_statute']=temp_df['temp_statute'].str.replace('986','496')
temp=temp_df['temp_statute'].str.split(stat_split[s],expand=True)
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['No','Yes','No','No','Financial limit obs. with no statute.']
checklist.to_csv('checklist_v1.csv',index=False)
...
#Get statute text.
for i in temp.index:
    if temp.loc[i,'section_url']!='432c':
        url=temp.loc[i,'url']
        print(f"Fetching: {url}")
        req  = urllib.request.Request(url, headers={'User-Agent': user_agent})
        page = urlopen(req)
        ...
        temp.loc[i,'statute_text']=''.join(text[0])
        if len(text)==2:
            temp.loc[i,'statute_mods']='[L'+''.join(text[1])
        else:
            temp.loc[i,'statute_mods']=''
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['No','No','No','No','One obs. cites entire chapter of 432C.']
checklist.to_csv('checklist_v1.csv',index=False)
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['No','No','No','Yes','No']
checklist.to_csv('checklist_v1.csv',index=False)

# %%Illinois. One obs. cites entire act: 805 ILCS 415.
stat_split['illinois']='/'
s='illinois'
url_state_year=s+'/2023/'
temp_df=current_df.loc[current_df.state==s]
...
for i in temp.index:
    if pd.isnull(temp.loc[i,'url'])==False:
        ...
        #This gets the universal citation.
        temp.loc[i,'universal_citation']=temp.loc[i,'chapter']+' ILCS '+temp.loc[i,'act']+'/'+temp.loc[i,'section']
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['No','No','No','No','One obs. cites entire act: 805 ILCS 415']
checklist.to_csv('checklist_v1.csv',index=False)
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['Yes','Yes','No','No','No']
checklist.to_csv('checklist_v1.csv',index=False)
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['No','No','No','Yes','One obs. only has the audit threshold. Another has multiple statutes, but no MULTI flag. A third has YWCA in place of the statute code.']
checklist.to_csv('checklist_v1.csv',index=False)
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['No','Yes','Yes','No','No']
checklist.to_csv('checklist_v1.csv')
...
temp_df.loc[temp_df.backsources_2023=='La. Rev. Stat. Ann. 40:2115.11','temp_statute']='40:2115.11'
temp=temp_df['temp_statute'].str.split(stat_split[s],expand=True)
temp.columns=['title','section']
temp['state']=s
temp=temp[['state','title','section']]
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['No','No','No','Yes - index 927 should have a hyphen between statute codes, and there is no multi flag','One obs. only says "At the time the organization is required to register.", but does not have statute code.']
checklist.to_csv('checklist_v1.csv',index=False)
...
temp['url']=url_base+url_state_year+temp['url']
temp.loc[((temp.section_url=='nan') | (temp_df.MULTI=='CHECK §§')),'url']=np.nan
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['No','No','No','Yes','One obs. backsources says "No* But the Attorney General may require a copy of any audited financial statement upon request (or a balance sheet if no audited statement)."']
checklist.to_csv('checklist_v1.csv',index=False)

# %%Maryland. One obs. cites all of Title 6.5.
stat_split['maryland']='-'
s='maryland'
url_state_year=s+'/2023/'
temp_df=current_df.loc[current_df.state==s]
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['No','No','No','Yes','One obs. cites all of Title 6.5']
checklist.to_csv('checklist_v1.csv',index=False)
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['No','No','No','Yes','One obs. only has the audit threshold.']
checklist.to_csv('checklist_v1.csv',index=False)
...
temp.loc[temp.chapter=='nan',['chapter','section']]=['0','0']
temp['act_url']='statute-act-169-of-1975'
temp.loc[temp.chapter=='0','act_url']=np.nan
temp.loc[((temp.chapter=='14') & (temp.section=='251')),'act_url']='statute-act-101-of-1961'
temp.loc[((temp.chapter=='14') & (temp.section=='304')),'act_url']='statute-act-298-of-1992'
temp.loc[((temp.chapter.astype('int')==450) & (temp.section.astype('int')>=251) & (temp.section.astype('int')<=253)),'act_url']='statute-act-169-of-1965'
temp.loc[((temp.chapter.astype('int')==450) & (temp.section.astype('int')>=2101) & (temp.section.astype('int')<=3192)),'act_url']='statute-act-162-of-1982'

temp['division_url']=''
temp.loc[((temp.chapter.astype('int')==450) & (temp.section.astype('int')>=2601) & (temp.section.astype('int')<=2651)),'division_url']='/division-162-1982-6'
temp.loc[((temp.chapter.astype('int')==450) & (temp.section.astype('int')>=2701) & (temp.section.astype('int')<=2754)),'division_url']='/division-162-1982-7'
temp.loc[((temp.chapter.astype('int')==450) & (temp.section.astype('int')>=2801) & (temp.section.astype('int')<=2864)),'division_url']='/division-162-1982-8'

temp['url']='chapter-'+temp['chapter']+'/'+temp['act_url']+temp['division_url']+'/section-'+temp['section_url']+'/'
temp['url']=url_base+url_state_year+temp['url']

#Get statute text.
for i in temp.index:
    if pd.isnull(temp.loc[i,'act_url'])==False:
        url=temp.loc[i,'url']
        print(f"Fetching: {url}")
        req  = urllib.request.Request(url, headers={'User-Agent': user_agent})
        page = urlopen(req)
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['No','Yes','No','No','Two obs. with no statute code associated with info regarding regulation']
checklist.to_csv('checklist_v1.csv',index=False)


# %%Minnesota. One obs. only has the audit threshold. 
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['No','No','No','No','One obs. only has the audit threshold.']
checklist.to_csv('checklist_v1.csv',index=False)
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['No','No','No','Yes','One obs. cites Miss. Code Ann. § 79-11-5, which is not a single statute.']
checklist.to_csv('checklist_v1.csv',index=False)
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['No','Yes','No','No','One obs. only has this UL, https://www.ago.mo.gov/docs/default-source/pdf-forms/profundraiserrenew.pdf?sfvrsn=2']
checklist.to_csv('checklist_v1.csv',index=False)
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['No','No','No','Yes','One obs. just says NONE.']
checklist.to_csv('checklist_v1.csv',index=False)
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['Yes','Yes','No','No','One obs. only has URL: http://nvsos.gov/index.aspx?page=113']
checklist.to_csv('checklist_v1.csv',index=False)
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['No','Yes','No','No','Some obs. have a URL: http://www.njconsumeraffairs.gov/charities']
checklist.to_csv('checklist_v1.csv',index=False)
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['No','Yes','No','No','Annotations after the amendments.  How should we deal with them?']
checklist.to_csv('checklist_v1.csv',index=False)
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['No','No','No','Yes','One of the nan obs. does not have the statute anywhere, just Yes. The other names all of article 9.']
checklist.to_csv('checklist_v1.csv',index=False)
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['No','No','No','Yes','One obs. cites https://www.secretary.state.nc.us/csl/. The other missing obs. has text instead of statute. One obs. only has the chapter 131F.']
checklist.to_csv('checklist_v1.csv',index=False)

# %%North Dakota. Do by hand for now.
stat_split['north dakota']='-' 
s='north dakota'
url_state_year=s.replace(' ','-')+'/2023/'
temp_df=current_df.loc[current_df.state==s]
print(np.sum(temp_df['MULTI']=='CHECK §§'))
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['No','No','Yes','No','No']
checklist.to_csv('checklist_v1.csv',index=False)
...
temp.loc[temp_df.backsources_2023=='Ohio Administrative Code 109:1-1-02(B)(1)','paragraph']='(B)(1)'
temp=temp.applymap(lambda x: x.lower() if isinstance(x, str) else x)
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['No','Yes','No','No','One obs. only has 522 for the section code, but this is not an individual statute.  One obs. only has UL: Oklahoma Secretary of State https://www.sos.ok.gov/(S(rxow5nrnwbjxrg45rwhhhg45))/charity/Default.aspx']
checklist.to_csv('checklist_v1.csv',index=False)

# %%Oregon. Multi issue. One missing obs. cites this URL https://www.doj.state.or.us/charitable-activities/for-professional-fundraisers/for-professional-fundraisers/.
stat_split['oregon']='.'
s='oregon'
url_state_year=s+'/2023/'
...
temp_df.loc[((temp_df.temp_statute=='nan') & (temp_df.backsources_2023.str.contains('O.R.S.'))),'temp_statute']=temp_df['backsources_2023'].str.extract(r'(\d+.\d+).')[0]
temp=temp_df['temp_statute'].str.split(stat_split[s],expand=True)
temp.columns=['chapter','section']
temp['volume']='02'
temp.loc[temp.chapter=='128','volume']='03'
temp['state']=s
temp=temp[['state','volume','chapter','section']]
...
temp['section_url']=temp['section_url'].str.rstrip()
temp=temp.applymap(lambda x: x.lower() if isinstance(x, str) else x) 
temp['section_url']=temp['chapter']+'-'+temp['section_url']
temp.loc[temp.chapter.str.len()==2,'chapter']='0'+temp['chapter']

temp['url'] = 'volume-'+temp['volume']+'/chapter-'+temp['chapter']+'/'+'section-'+temp['section_url']+'/'
temp['url']=url_base+url_state_year+temp['url']
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['No','No','No','Yes','One obs. has the URL: https://www.doj.state.or.us/charitable-activities/for-professional-fundraisers/for-professional-fundraisers/']
checklist.to_csv('checklist_v1.csv',index=False)

# %%Pennsylvania.  May need to pull from findlaw manually. Pa. O.C. Rule 5.5 pulled from external website by hand. Title 10 on findlaw.
...
temp=temp_df['temp_statute'].str.split(stat_split[s],expand=True)
temp.columns=['temp_chapter','temp_section']
temp['state']=s
temp['title']=np.nan
temp.loc[temp_df.temp_statute!='nan','title']=temp_df['backsources_2023'].str.extract(r'(\d+) P', expand=False)
temp.loc[temp.temp_chapter.str.len()==3,'chapter']=temp['temp_chapter'].str[0]
temp.loc[temp.temp_chapter.str.len()==4,'chapter']=temp['temp_chapter'].str[:2]
temp.loc[temp.title=='10','chapter']='6'
temp.loc[temp.title=='10','temp_title']='10'

temp.loc[((temp_df.backsources_2023.str.contains('Pa. Orphan'))|(temp_df.backsources_2023.str.contains('Pa. O.C.'))),'temp_section']='5.5'

temp['section']=temp_df['temp_statute']
temp.loc[temp.section=='732-204',['title','temp_title']]=['71-ps-state-government','71']

temp.loc[((temp.title=='15')|(temp.title=='20')),'site']='justia'
#temp.loc[temp.index==1951,'site']='findlaw'
temp.loc[temp.title=='10','site']='findlaw'
temp.loc[temp.section=='732-204','site']='findlaw'
temp.loc[temp.temp_section=='5.5','site']=''
temp.loc[temp.temp_section=='5.5','url']='https://evans-legal.com/dan/ocrules/paoc05.html'
temp.loc[temp.title=='10','title']='10-ps-charities-and-welfare'
...
for i in temp.index:
    if temp.loc[i,'site']=='justia':
        ...
        k=0
        for j in text:
            temp_text.append(j.get_text())
            if ((j.get_text().find('Amendment. Act')!=-1) & (pd.isnull(split_loc)==True)):
                split_loc=k
            k+=1
        
        if pd.isnull(split_loc)==False:
            temp.loc[i,'statute_text']=''.join(temp_text[:split_loc])
            temp.loc[i,'statute_mods']=''.join(temp_text[split_loc:])
        else:
            temp.loc[i,'statute_text']=''.join(temp_text)
            temp.loc[i,'statute_mods']=''
        temp.loc[i,'universal_citation']=soup.find('div',class_='citation').find('span').get_text()
    elif temp.loc[i,'site']=='findlaw':
        url=temp.loc[i,'url']
        print(f"Fetching: {url}")
        ...
        html = page.read().decode("utf-8")
        soup = BeautifulSoup(html, "html.parser")
        text=soup.find('div',class_='codes-content__text codes-content__text--min-height').get_text().replace('\t','').replace('\n','').replace('   ',' ')
        temp.loc[i,'statute_text']=text
        temp.loc[i,'statute_mods']=''
        if temp.loc[i,'title']=='10-ps-charities-and-welfare':
            temp.loc[i,'universal_citation']='10 P.S. § '+temp.loc[i,'section']
        elif temp.loc[i,'title']=='71-ps-state-government':
                temp.loc[i,'universal_citation']='71 P.S. § '+temp.loc[i,'section']
        if soup.find('p',class_='fl-block-paragraph').get_text().find('January 01, 2024')!=-1:
            temp.loc[i,'universal_citation']=temp.loc[i,'universal_citation']+' (2023)'
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['No','No','Yes','Yes - index 1951 might have a multi issue','Had to use a combination of Justia, Findlaw, and manual. One obs. has the following list of org. types, but no clear statute code: Law enforcement (162.3), volunteer firefighters (3), libraries (4), nursing homes (5),']
checklist.to_csv('checklist_v1.csv',index=False)


# %%Rhode Island.  Simplify later.
...
# Fetch statute text
for i in temp.index:
    if temp.loc[i,'temp_statute']!='nan':
        url=temp.loc[i,'url']
...
temp=temp[['state','title','chapter','section','paragraph','url','statute_text','statute_mods','universal_citation']]
...
# %%South Dakota. One obs. has 37-30-36 as the section code, doesn't have info on statute text. Checked the language, and 36 should be 26. SDCL 47-25A does not have a section.
...
print(np.sum(temp_df['MULTI']=='CHECK §§'))
print(temp_df.loc[temp_df.temp_statute=='nan','backsources_2023'])
temp_df.loc[((temp_df.temp_statute=='nan') & (temp_df.backsources_2023!='NONE')),'temp_statute']=temp_df['backsources_2023'].str.extract(r'(\d+-\d+A)')[0]
temp_df.loc[temp_df.temp_statute=='37-30-36','temp_statute']='37-30-26'
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['No','No','No','No','One obs. with SDCL 47-25A does not have a section.']
checklist.to_csv('checklist_v1.csv',index=False)

# %%Tennessee. Multi issue. One obs. only has audit threshold, no statute. One obs. only has this URL http://sos.tn.gov/charitable. Two obs. have no information.
...
temp=temp[['state', 'title', 'chapter', 'section', 'paragraph', 'section_url','part','url', 'statute_text', 'statute_mods','universal_citation']]
temp.reset_index(inplace=True)
main_df=pd.concat([main_df,temp],ignore_index=True)
main_df.to_csv('current_text_v1.csv',index=False)

#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['No','No','No','Yes','Two obs. mention exemptions, but do not have a statute code.']
checklist.to_csv('checklist_v1.csv',index=False)
...
temp_df.loc[temp_df.temp_statute=='nan','temp_statute']=temp_df['backsources_2023'].str.extract(r'(\d+.\d+)')[0]
...
temp=temp[['state','title','chapter', 'subchapter','section', 'section_url', 'paragraph','url', 'statute_text', 'statute_mods','universal_citation']]
temp.reset_index(inplace=True)
main_df=pd.concat([main_df,temp],ignore_index=True)
main_df.to_csv('current_text_v1.csv',index=False)

#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['Yes','No','No','No','No']
checklist.to_csv('checklist_v1.csv',index=False)
...
# %%Virginia. Need to fix. Multi issue. VA Code (Ann.) § 55-531 and 55-532 were repealed 10/1/2019. It was replaced by VA Code § 32.1-373 and 32.1-374.
stat_split['virginia']='-'  # Changed from '.' to '§' for Virginia
s='virginia'
url_state_year=s+'/2023/'  # Fixed: removed '/2023/' for Virginia
temp_df=current_df.loc[current_df.state==s]
print(np.sum(temp_df['MULTI']=='CHECK §§'))
print(temp_df.loc[temp_df.temp_statute=='nan','backsources_2023'])

#Replace repealed statutes with correct statutes.
temp_df.loc[temp_df.temp_statute=='55-531','temp_statute']='32.1-373'
temp_df.loc[temp_df.temp_statute=='55-532','temp_statute']='32.1-374'
...
temp=temp[['state','title','section']]
temp['chapter']='5'
temp.loc[temp.title=='13.1','chapter']='10'
temp.loc[temp.title=='32.1','chapter']='20'
temp.loc[temp.title=='57','chapter']='5'
temp.loc[temp.title=='64.2','chapter']='7'
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['No','No','No','Yes','One obs. has audit threshold, but no other audit statutes. Index 2305 has a multi issue that is not coded.']
checklist.to_csv('checklist_v1.csv',index=False)
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['No','No','Yes','Yes','Do WACs by hand. They are in an archive here: https://lawfilesext.leg.wa.gov/law/WACArchive/2023/WAC%20434%20-120%20%20CHAPTER.htm']
checklist.to_csv('checklist_v1.csv',index=False)
...
#Update checklist.
checklist.loc[checklist.state==s,['Completed','Paragraph Ready','Manual','Multi Issue','Other']]=['Yes','No','No','No','One obs. has no information.']
checklist.to_csv('checklist_v1.csv',index=False)
